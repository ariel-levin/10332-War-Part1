...
public class LauncherDestroyer extends Thread {
				...
				if (l != null) {		// if launcher was found on the Heap's head
					...
					if ( war.getTime() >= nextDestroy ) {
						...
						synchronized (this) {
							...
							if (t.isSetDuringWar())
								destroyLauncherWithDelayAndSuccess(l);
							else
	...
	/** Destroy the input Launcher with action delay and success chance.
	 * Used when the destruction is set after War has started */
	private void destroyLauncherWithDelayAndSuccess(Launcher l) {
