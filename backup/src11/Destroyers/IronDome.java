...
public class IronDome extends Thread {
				...
				if (m != null) {		// if missile was found on the Heap's head
					...
					if ( war.getTime() >= nextInterception ) {
						...
						synchronized (this) {
							...
							if (t.isSetDuringWar())
								interceptMissileWithDelayAndSuccess(m);
							else
	...
	/** Intercept the input Missile with action delay and success chance.
	 * Used when the interception is set after War has started */
	private void interceptMissileWithDelayAndSuccess(Missile m) {
